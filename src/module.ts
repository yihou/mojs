...
export class Module {
  ...
  _setProp(attr, value) {
    if (typeof attr === 'object') {
      for (const key in attr) { this._assignProp(key, attr[key]); }
  ...
  */
  ...
  _parseDeltaValues(name, delta) {
    ...
    for (const key in delta) {
      ...
      d[this._parsePreArrayProperty(name, key)] = this._parsePreArrayProperty(name, value);
    }
    return d;
  }
  ...
  _calcCurrentProps(easedProgress, p) {
    ...
    for (const key in this._deltas) {
      ...
      const ep = (value.easing != null && !isCurve)
      ...
      if (value.type === 'array') {
        ...
        if (!isCurve) {
          ...
          r = parseInt(cp * (value.start.r + p * value.delta.r) as any as string, 10);
          g = parseInt(cp * (value.start.g + p * value.delta.g) as any as string, 10);
          b = parseInt(cp * (value.start.b + p * value.delta.b) as any as string, 10);
          a = parseFloat(cp * (value.start.a + p * value.delta.a) as any as string);
        }
