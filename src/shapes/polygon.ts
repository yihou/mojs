  ...
  */
  _draw() {
    ...
    if ( !( isRadiusX && isRadiusY && isPoints) ) {
      ...
      if ((this._radialPoints == null)) { this._radialPoints = [];
      } else { this._radialPoints.length = 0; }

      for (i = 0, end = this._props.points, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
  ...
  /*
    ...
    @overrides @ Bit
  */
  _getLength() { return this._getPointsPerimiter( this._radialPoints ); }
}
