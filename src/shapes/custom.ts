...
class Custom extends Bit {
  ...
  */
  _declareDefaults() {
    ...
    for (let i = 0; i < this._drawMap.length; i++) {
  ...
  */
  _getScale() {
    const p = this._props
    const radiusX = (p.radiusX) ? p.radiusX : p.radius
    const radiusY = (p.radiusY) ? p.radiusY : p.radius
    ...
    const translate = `translate(${p.shiftX}, ${p.shiftY})`
