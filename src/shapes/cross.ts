...
class Cross extends Bit {
  ...
  _draw() {
    ...
    if ( isRadiusX && isRadiusY ) { return; }

    const x = this._props.width/2; const y = this._props.height/2;
    const x1 = x-radiusX; const x2 = x+radiusX;
    const line1 = `M${x1},${y} L${x2},${y}`;
    const y1 = y-radiusY; const y2 = y+radiusY;
