...
DOMNode or another module.`)
    ...
    if (o) {
      for (const key in o) {
        ...
        if (h.callbacksMap[key] || h.tweenOptionMap[key]) {
          h.warn(
            `the property "${key}" property can not be overridden on run yet`
          )
          delete o[key]
        } else {
  ...
  setProgress(p, isInit = false) {
    const len =
      this.startLen +
      (!this.props.isReverse ? p * this.slicedLen : (1 - p) * this.slicedLen)
  ...
  _getCurrentRotation(point, len, p) {
    ...
    if (
      ...
      if (typeof this.props.rotationOffset !== 'function') {
        ...
        return (this.rotate = this.props.rotationOffset.call(
          this,
          this.rotate,
          p
        ))
  ...
  _setTransformOrigin(p) {
    if (this.props.transformOrigin) {
      ...
      const tOrigin = !isTransformFunOrigin
