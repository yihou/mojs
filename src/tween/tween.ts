...
class Tween extends Module {
  ...
  */
  _setSelfName() {
    ...
    tweener[globalName] = (tweener[globalName] == null) ? 1 : ++tweener[globalName];
  ...
  */
  _updateInActiveArea(time) {
    ...
    const TCount = Math.round((props.endTime - props.startTime + props.delay) / delayDuration)
  ...
  */
  ...
  _setProgress(proc, time, isYoyo) {
    ...
    // get the current easing for `backward` direction regarding `yoyo`
      ...
      const easing = (p.backwardEasing != null)
        ? p.backwardEasing
        : p.easing
